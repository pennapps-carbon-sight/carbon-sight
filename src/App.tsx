...
import apiService, { type ChatRequest, type ChatResponse, type ModelComparisonResponse } from "./api";
...
function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!supabase) {
      // If Supabase is not configured, enable mock mode
      setMockMode(true);
      return;
    }
    
    // For development, enable mock mode by default
    setMockMode(true);
  ...
  const mockSession = mockMode ? {
    user: {
      id: "mock-user-123",
      email: "demo@carbonsight.ai",
  ...
  async function sendOTP(email: string) {
    if (mockMode) {
      console.log("[Auth] Mock mode - simulating OTP send to:", email);
      return;
    }
  ...
  async function verifyOTP(email: string, token: string) {
    if (mockMode) {
      console.log("[Auth] Mock mode - simulating OTP verification for:", email);
  ...
  async function logout() { 
    if (mockMode) {
      console.log("[Auth] Mock mode - simulating logout");
      setSession(null);
      return;
    }
  ...
  const value = useMemo(() => ({ 
    session: mockMode ? mockSession : session, 
    user: (mockMode ? mockSession : session)?.user ?? null, 
    sendOTP, 
    verifyOTP, 
    logout 
  }), [session, mockMode, mockSession]);
...
type Energy = "sustainable" | "balanced" | "intensive" | "smart";
type Model = { id: string; label: string; energy: Energy };

const MODELS: Model[] = [
  { id: "auto", label: "Auto (Best Model)", energy: "smart" },
  { id: "gemini-2.5-flash", label: "Gemini 2.5 Flash", energy: "sustainable" },
  { id: "gemini-2.5-flash-lite", label: "Gemini 2.5 Flash Lite", energy: "sustainable" },
  { id: "gemini-1.5-flash", label: "Gemini 1.5 Flash", energy: "balanced" },
...
<section className="relative mt-14 -mx-4 md:-mx-8 lg:-mx-12">
  <div className="mx-auto w-full max-w-[1400px]">
    ...
    <NeuralEnergyWeb models={WEB_MODELS as any} height={640} />
  </div>
</section>
        ...
        <div className="space-y-4 px-6 py-6 sm:px-8">
          ...
          {step === 'email' && (
            <form onSubmit={handleSendOTP} className="space-y-4">
              ...
              {/* Skip Authentication Button for Development */}
              <button
                type="button"
                onClick={handleSkipAuth}
                className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-slate-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-500"
              >
...
function ChatScreen() {
  ...
  const [messages, setMessages] = useState<{ role: "user" | "assistant"; content: string }[]>([
    { role: "assistant", content: "Hi! Ask me anything about CarbonSight." },
  ]);
  ...
  const [lastEnergyData, setLastEnergyData] = useState<{model: string, energy: number, co2: number, cost: number, credits: number} | null>(null);
  ...
  async function sendMessage() {
    ...
    // Show loading state
    setMessages((m) => [...m, { role: "assistant", content: "🤖 Generating response..." }]);

    try {
      // Determine which model to use
      const modelToUse = autoSwitch ? "auto" : selectedModel;
      
      // First, get a normal AI response using the selected model
      ...
      // Store energy data for display
      setLastEnergyData({
        model: response.energy_metrics.model_name, // Use the actual model that was used
        energy: response.energy_metrics.energy_kwh,
        co2: response.energy_metrics.co2_grams,
        cost: response.tokens_used.total * 0.0001, // Approximate cost calculation
        credits: response.energy_metrics.co2_grams * 0.02 // $GREEN credits earned
      });
      
      // Update with normal AI response
      setMessages((m) => {
        const newMessages = [...m];
      ...
      // Now run the 6-model comparison in the background
      setMessages((m) => [...m, { role: "assistant", content: "🔍 Analyzing efficiency across all 6 Gemini models..." }]);

      const modelComparison = await apiService.testAllModels(
        text, 
        user?.email || "anonymous", 
        "team-engineering"
      );

      if (modelComparison.success) {
        // Create detailed stats table
        const statsTable = createModelComparisonTable(modelComparison.model_comparison);
        
        setMessages((m) => {
          const newMessages = [...m];
          newMessages[newMessages.length - 1] = { 
            role: "assistant", 
            content: `## 📊 **Model Efficiency Analysis**
...
**🌱 Environmental Impact:** The recommended model uses ${getEnergySavings(modelComparison)} less energy and produces ${getCO2Savings(modelComparison)} less CO2 emissions compared to the least efficient option.

**💰 Cost Savings:** The recommended model costs ${getCostSavings(modelComparison)} less per request compared to the most expensive option.

**⚡ Performance:** ${getPerformanceSummary(modelComparison)}` 
          };
          return newMessages;
        });

        // Trigger visual effect based on best model efficiency
        ...
        // Remove loading message if comparison fails
        setMessages((m) => m.slice(0, -1));
      }
      
    } catch (error) {
      console.error("[Chat] Error calling backend API:", error);
      setMessages((m) => {
        const newMessages = [...m];
        newMessages[newMessages.length - 1] = { 
          role: "assistant", 
...
Sorry, I encountered an issue processing your request. Please try again.` 
  ...
  function createModelComparisonTable(modelComparison: any) {
    const models = Object.entries(modelComparison).sort((a, b) => (b[1] as any).efficiency_score - (a[1] as any).efficiency_score);
    
    return `| Model | Latency | Energy | CO₂ | Cost | Efficiency | Status |
|-------|---------|--------|-----|------|------------|--------|
${models.map(([model, stats]: [string, any]) => {
  const latency = `${stats.latency_ms}ms`;
  const energy = `${(stats.energy_kwh * 1000).toFixed(1)}Wh`;
  const co2 = `${stats.co2_grams.toFixed(3)}g`;
  const cost = `$${(stats.cost_usd || 0).toFixed(4)}`;
  const efficiency = `${(stats.efficiency_score * 100).toFixed(1)}%`;
  const status = stats.success ? '✅' : '❌';
  const isBest = model === models[0][0] ? ' ⭐' : '';
  
  return `| \`${model}\`${isBest} | ${latency} | ${energy} | ${co2} | ${cost} | ${efficiency} | ${status} |`;
}).join('\n')}`;
  }

  // Helper function to calculate energy savings
  function getEnergySavings(comparison: any) {
    const models = Object.values(comparison.model_comparison) as any[];
    const maxEnergy = Math.max(...models.map(m => m.energy_kwh || 0));
    const minEnergy = Math.min(...models.map(m => m.energy_kwh || 0));
    
    // Handle case where all energy values are the same or maxEnergy is 0
    if (maxEnergy === 0 || maxEnergy === minEnergy) {
      return "0%";
    }
    
    const savings = ((maxEnergy - minEnergy) / maxEnergy * 100).toFixed(1);
    return `${savings}%`;
  }

  // Helper function to calculate CO2 savings
  function getCO2Savings(comparison: any) {
    const models = Object.values(comparison.model_comparison) as any[];
    const maxCO2 = Math.max(...models.map(m => m.co2_grams || 0));
    const minCO2 = Math.min(...models.map(m => m.co2_grams || 0));
    
    // Handle case where all CO2 values are the same or maxCO2 is 0
    if (maxCO2 === 0 || maxCO2 === minCO2) {
      return "0%";
    }
    
    const savings = ((maxCO2 - minCO2) / maxCO2 * 100).toFixed(1);
    return `${savings}%`;
  }

  // Helper function to calculate cost savings
  function getCostSavings(comparison: any) {
    const models = Object.values(comparison.model_comparison) as any[];
    const maxCost = Math.max(...models.map(m => m.cost_usd || 0));
    const minCost = Math.min(...models.map(m => m.cost_usd || 0));
    
    // Handle case where all costs are the same or maxCost is 0
    if (maxCost === 0 || maxCost === minCost) {
      return "0%";
    }
    
    const savings = ((maxCost - minCost) / maxCost * 100).toFixed(1);
    return `${savings}%`;
  }

  // Helper function to get performance summary
  function getPerformanceSummary(comparison: any) {
    const models = Object.values(comparison.model_comparison) as any[];
    const avgLatency = (models.reduce((sum, m) => sum + m.latency_ms, 0) / models.length).toFixed(0);
    const fastestModel = models.reduce((fastest, current) => current.latency_ms < fastest.latency_ms ? current : fastest);
    return `Average response time: ${avgLatency}ms. Fastest: ${fastestModel.latency_ms}ms`;
  }
  ...
  const energyChip = {
    ...
    smart:       { dot: "bg-purple-400",  text: "text-purple-300",  bg: "bg-purple-500/10 ring-purple-400/30",  label: "Smart Auto" },
  }[currentModel.energy];
      ...
      <section className="relative isolate flex min-w-0 flex-1 flex-col">
            ...
            return (
              <div key={i} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                <div className="max-w-[72ch]">
                  ...
                  {!mine && isLastAssistant && autoSwitch && lastEnergyData && (
                    <div className="mt-2 flex items-center gap-2 rounded-lg bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300">
                      <span>⚡</span>
                      <span>Used: {lastEnergyData.model}</span>
                      <span>•</span>
                      <span>{lastEnergyData.energy.toFixed(3)} kWh</span>
                      <span>•</span>
                      <span>{lastEnergyData.co2.toFixed(2)}g CO₂</span>
                      <span>•</span>
                      <span>${lastEnergyData.credits.toFixed(3)} $GREEN earned</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <footer className="relative z-20 border-t border-white/10 p-3">
          <div className="mx-auto max-w-4xl">
            {/* Auto-Switch Toggle and Model Picker */}
            <div className="mb-4 flex items-center justify-between rounded-xl border border-white/10 bg-black/20 p-4">
              <div className="flex items-center gap-4">
                {/* Auto-Switch Toggle */}
                <div className="flex items-center gap-2">
                  <label className="text-sm font-medium text-slate-300">Auto-Switch</label>
                  <button
                ...
                {/* Model Picker (only shown when auto-switch is OFF) */}
                {!autoSwitch && (
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="rounded-lg border border-white/20 bg-black/50 px-3 py-1 text-sm text-white"
                    >
                      {MODELS.map((model) => (
              ...
              {/* Energy Badge (only shown when auto-switch is OFF) */}
              {!autoSwitch && selectedModel !== "auto" && (
                <div className={`flex items-center gap-2 rounded-lg px-3 py-1 text-sm ${
                  ...
                  {selectedModel.includes('pro') ? (
                    <>
                      <span>🔴</span>
                      <span>High Energy Model (Pro) — consider switching.</span>
                    </>
                  ) : (
                    <>
                      <span>🔵</span>
                      <span>Efficient Model (Flash) — Green Choice.</span>
                    </>
                  )}
                </div>
              )}
